...
export function stepExpected(testCase: TestCase, stepIndex: number) {
  ...
  if (/^open the request type configuration page/i.test(step)) return "The Request Type Configuration page opens successfully.";
  if (/^click the new request type button/i.test(step)) return "The new request-type form is displayed.";
  if (/^enter .+ in the request type name field/i.test(step)) return "The request-type name appears in the Request Type Name field without validation errors.";
  if (/^leave the request type name field empty/i.test(step)) return "The Request Type Name field remains empty and is ready to show a required-field message.";
  if (/^select .+ in the workflow field/i.test(step)) return "The selected workflow appears in the Workflow field.";
  if (/^leave the workflow field empty/i.test(step)) return "The Workflow field remains empty and is ready to show a required-field message.";
  if (/^select active in the status dropdown/i.test(step)) return "Active appears in the Status dropdown.";
  if (/^select inactive in the status dropdown/i.test(step)) return "Inactive appears in the Status dropdown.";
  if (/^click the save button on the request type configuration page/i.test(step)) return "The request-type form is submitted for validation and save processing.";
  if (/^open the employee request list/i.test(step)) return "The employee request list opens and displays available request-type options.";
  if (/^select .+ from the employee request type list/i.test(step)) return "The selected request type appears in the employee request form.";
  if (/^open an existing .+ request type record/i.test(step)) return "The existing request-type record opens with its saved workflow and status visible.";
  if (/^change the status dropdown from active to inactive/i.test(step)) return "Inactive appears in the Status dropdown before saving.";
  if (/^change the status dropdown from inactive to active/i.test(step)) return "Active appears in the Status dropdown before saving.";
  if (/^open the inactive request type url directly/i.test(step)) return "The direct inactive request-type URL request is made for authorization and availability validation.";
  if (/^sign in as a user without request type configuration permission/i.test(step)) return "The lower-privilege session opens without request-type configuration permission.";
  if (/^open the protected request type configuration page/i.test(step)) return "The protected configuration page request is blocked or opens without edit controls.";
  if (/^attempt to create a request type through the backend api/i.test(step)) return "The unauthorized backend request is submitted with request-type configuration data.";
  if (/^simulate workflow engine unavailable/i.test(step)) return "The Workflow Engine dependency unavailable condition is active before the request is submitted.";
  if (/^simulate ess request module unavailable/i.test(step)) return "The ESS Request Module dependency unavailable condition is active before the request is submitted.";
